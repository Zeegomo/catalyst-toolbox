...
fn load_tickets_from_reviews(
    ...
    let excellent_reviews = proposal_reviews
        .iter()
        .filter(|rev| matches!(rev.score(), ReviewRanking::Excellent))
        .count() as u64;
    let good_reviews = proposal_reviews
        .iter()
        .filter(|rev| matches!(rev.score(), ReviewRanking::Good))
        .count() as u64;
    ...
    let ticket_distribution = proposal_reviews
        ...
        .map(|rev| {
            let tickets = match rev.score() {
                ReviewRanking::Excellent => rewards_slots.excellent_slots,
                ReviewRanking::Good => rewards_slots.good_slots,
